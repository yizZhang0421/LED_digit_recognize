...
cv2.imshow('test', get_show_image(img))
cv2.waitKey(wait_time)

gray=img[:, :, 0]
cv2.imshow('test', get_show_image(gray))
cv2.waitKey(wait_time)

blurred = cv2.GaussianBlur(gray, (11, 11), 0)

binary = cv2.Canny(blurred, 10, 100)
cv2.imshow('test', get_show_image(binary))
cv2.waitKey(wait_time)
...
cv2.imshow('test', get_show_image(tmp))
cv2.waitKey(wait_time)
...
cv2.imshow('test', get_show_image(test))
cv2.waitKey(wait_time)
